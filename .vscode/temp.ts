...
import { is, isBoolean, isLast, isLastAt, isNumber, isOf, isUndefined, not, ws } from 'utils/helpers';
...
export function markup () {
   ...
   */
  const c = (ender < 1 || ender > data.token.length) ? data.token.length : ender + 1;
   ...
   * Spaces
   *
   * Generate the indentation level and character indentation spacing to be applied.
   */
  function spaces () {

    const indentChar = [ rules.indentChar ];
    const indentSize = rules.indentSize - 1;

    let aa = 0;

    if (aa < indentSize) {
      do {
        indentChar.push(rules.indentChar);
        aa = aa + 1;
      } while (aa < indentSize);
    }

    return indentChar.join(NIL);
  ...
  function onComment () {
    ...
    if (test === true) {
      ...
      if ((
        ...
        // Here we are encountering comment indentation for {% else %} or {% elsif %}
  ...
  function onContent () {
    ...
    if (n < c && (isIndex(n, 'end') > -1 || isIndex(n, 'start') > -1) && data.lines[n] > 0) {
  ...
  function getLevels () {
    ...
    //
    // r -> last token in data structure
      ...
      if (data.lexer[a] !== 'markup') {
        ...
        if (isIndex(a, 'attribute') > -1) {
          ...
          if (
            ...
            (rules.markup.forceIndent === true && isType(n, 'script_start'))
          ) && (
            isType(a, 'content') ||
            isType(a, 'singleton') ||
            isType(a, 'liquid')
            ...
            if (
              ...
              isType(n, 'liquid_end') ||
              isType(n, 'liquid_start')
            ) && (
              data.lines[n] > 0 ||
              isIndex(a, 'liquid_') > -1
            )) {

              if (isType(n, 'liquid_case_end') && !dedent.has('case')) indent = indent - 1;
            ...
            if (jsx === true && is(data.token[a + 1], cc.LCB)) {
              ...
              //
              if (data.stack[a] === 'liquid' || isIndex(n - 1, 'comment') > -1) {
            ...
            } else if (data.lines[n] < 2 && ( // changed from 0
              isType(n, 'content') ||
              isType(n, 'singleton') || (
                isType(a, 'start') &&
                isType(n, 'liquid') &&
              ...
              // console.log(data.token[a], indent);
          ...
          } else if (rules.markup.forceIndent === false && data.lines[n] === 0 && (
            isType(n, 'content') ||
            isType(n, 'singleton')
          )) {
          ...
          } else if (isType(n, 'liquid_bad_end')) {
          ...
          } else if (isType(r, 'liquid_start') && isType(n, 'liquid_end') && data.lines[n] === 0) {
  ...
  function onIgnoreRule () {
    ...
    /**
     * rious indentation reference, exclude newlines
     */
        ...
        if (
          ...
          if ((levels[a] === -10 && a < c - 1)) {
            ...
            if (
              ...
              isType(a + 1, 'comment') &&
              rules.markup.preserveComment === true &&
              isOf(data.token[a + 1].trimStart()[1], cc.BNG, cc.PER)) {
