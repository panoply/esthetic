...
When the `commentPreserve` (markup) rule is enabled it will take precedence and override `commentIndent` rule.
...
<div>
  ...
  <nav>
    <!-- Comments which span multiple lines will still
      respect wrap but will begin and end on the same
      line as the comment contents. This is generally
      not very elegant, but I'm not the boss of you! -->
    </nav>
</div>
```
...
<nav>
  <!-- This is the authors choice. It's rather
    ...
    some devs like comment delimiters inline. -->
</nav>
</div>
```
