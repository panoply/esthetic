        ...
        if (argument) {
          for (const [match] of EMBEDDED[tag].args) {
            ...
            if (isArray(argument)) {
              ...
              const exp2 = new RegExp(argument);
              if (match.size > 0) {
                for (const m of match) {
                  if (!(m instanceof RegExp))
                    continue;
                  if (m.source !== exp2.source)
                    match.add(exp2);
                }
              } else {
                match.add(exp2);
        ...
        if (attribute) {
          ...
          for (const attr in attribute) {
            ...
            if (isArray(attribute[attr])) {
              ...
              const exp2 = new RegExp(attribute[attr]);
...
function commentBlock(config) {
  ...
  if (isliquid === true && rules.liquid.preserveComment || isliquid === false && rules.markup.preserveComment || parse.lexer === "style" && rules.style.preserveComment || parse.lexer === "script" && rules.style.preserveComment || rules.wrap < 1 || a === config.end || output.length <= rules.wrap && output.indexOf(NWL) < 0 || config.begin === "/*" && output.indexOf(NWL) > 0 && output.replace(NWL, NIL).indexOf(NWL) > 0 && /\n(?!(\s*\*))/.test(output) === false) {
    return [output, a];
  }
  b = config.start;
...
function markup(input) {
  ...
  function indent(from, token = NIL) {
    ...
    if (nl > -1) {
      ws2 = source.slice(nl + 1, from);
      if (ws2.length > 0 && ws2.trim().length < 1)
        return token ? ws2 + token : ws2;
    }
    return token;
  }
  function newlines(from, input2 = b) {
    let i = from;
    let n = 1;
    while (is(input2[i++], 10 /* NWL */))
      n = n + 1;
    return n;
  }
  function glue(from, to = -1, noTrim = false) {
    if (!noTrim)
      return source.slice(from, to).trimStart();
  ...
  function parseToken(end) {
    ...
    function parseIgnore() {
      ...
      if (ender !== null && preserve === false && ignore === true && (end === ">" || end === "}}" || end === "%}")) {
        ...
        if (ltype === "ignore") {
          ...
          if (spacer === NIL) {
            const last = data.token[parse.count].search(NonSpace);
            spacer = last > 0 ? repeatChar(data.token[parse.count].search(NonSpace)) : NIL;
          }
          attrs = [];
    ...
    function parseComments(lineComment) {
      ...
      if (CommMarkupIgnore.test(token)) {
        if (CommIgnoreStart.test(token)) {
          let begin;
          if (token.startsWith("<!--")) {
            begin = token.indexOf("-->") + 3;
          } else if (LiquidLineComment.test(token)) {
          ...
          push(record, [
            ...
            {
              token: token.slice(begin).replace(NewlineLead, NIL),
              types: "ignore",
    ...
    function traverse() {
      ...
      function tokenize(quotes) {
        ...
        if (attrs.length > 0) {
          ...
          if (within === 0 && (is(attr, 61 /* EQS */) || is(attr, 45 /* DSH */))) {
                ...
                do {
                  ...
                  if (jsx === false && qattr === false && isliq === true && rules.markup.preserveAttributes === false) {
                    while (a < c) {
                      ...
                      if (is(b[a], 10 /* NWL */))
                        lines = parse.lines(a, lines);
                      if (is(store[0], 61 /* EQS */) && (is(store[1], 123 /* LCB */) || is(store[1], 37 /* PER */)) && (is(store[store.length - 2], 125 /* RCB */) || is(store[store.length - 2], 37 /* PER */)) && is(store[store.length - 1], 125 /* RCB */)) {
                        isliq = false;
                        quote = NIL;
                        tokenize(false);
                        break;
                      }
                      if (is(store[0], 61 /* EQS */) && not(store[1], 123 /* LCB */)) {
                        isliq = false;
                        quote = NIL;
                        tokenize(false);
                        break;
...
function script() {
  ...
  function parseMarkup() {
    ...
    next = peek(1, false);
    if (parse.language !== "jsx" && parse.language !== "tsx" && digit(next) === false && (ltoke === "function" || ptoke === "=>" || ptoke === "void" || ptoke === "." || ltoke === "return" || ltype === "operator" || data.stack[parse.count] === "arguments" || ltype === "type" && ptoke === "type" || ltype === "reference" && (ptype === "operator" || ptoke === "function" || ptoke === "class" || ptoke === "new") || ltype === "type" && ptype === "operator")) {
      const build = [];
      let i = 0;
      let e = 0;
      d = a;
    ...
    if (parse.language !== "typescript" && (data.token[d] === "return" || data.types[d] === "operator" || data.types[d] === "start" || data.types[d] === "separator" || data.types[d] === "jsx_attribute_start" || is(data.token[d], 125 /* RCB */) && parse.stack.token === "global")) {
      ltype = "markup";
      parse.language = "jsx";
      do {
        output.push(c[a]);
  ...
  function parseTokens(starting, ending, type) {
    ...
    function finish() {
      ...
      function bracketSpace(input) {
        if (parse.language !== "javascript" && parse.language !== "typescript" && parse.language !== "jsx" && parse.language !== "tsx") {
    ...
    if (ee < b) {
      do {
        ...
        if (parse.language !== "json" && parse.language !== "javascript" && (is(starting, 34 /* DQO */) || is(starting, 39 /* SQO */)) && (ext === true || ee > start2) && not(c[ee - 1], 92 /* BWS */) && not(c[ee], 34 /* DQO */) && not(c[ee], 39 /* SQO */) && (is(c[ee], 10 /* NWL */) || ee === b - 1 === true)) {
  ...
  function applyBrace() {
    ...
    const g = parse.count;
    const lines = parse.lineOffset;
    if (parse.language === "json" || brace.length < 1 || brace[brace.length - 1].charAt(0) !== "x" || /^x?(;|\}|\))$/.test(ltoke) === false) {
      return;
    }
  ...
  function word() {
    ...
    if (option.correct === true && (output === "Object" || output === "Array") && is(c[a + 1], 40 /* LPR */) && is(c[a + 2], 41 /* RPR */) && is(data.token[parse.count - 1], 61 /* EQS */) && data.token[parse.count] === "new") {
      ...
      if (output === "void") {
      ...
      } else if (tstype[tstype.length - 1] === true || (parse.language === "typescript" || parse.language === "flow") && tokel === "type") {
        ltype = "type";
        ...
        if (parse.language === "javascript" || parse.language === "jsx" || parse.language === "typescript" || parse.language === "tsx") {
        ...
        } else if (references.length > 0 && (data.token[d] === "let" || data.token[d] === "const" || data.token[d] === "type" && (parse.language === "typescript" || parse.language === "tsx"))) {
          ltype = "reference";
          references[references.length - 1].push(output);
          hoisting(d, output, false);
        } else {
    ...
    function selector(index) {
      ...
      if (is(data.token[ss - 1], 44 /* COM */) || is(data.token[ss - 1], 58 /* COL */) || data.types[ss - 1] === "comment" || data.types[ss - 1] === "pseudo") {
        if (data.types[ss - 1] === "colon" && (data.types[ss] === "selector" || data.types[ss] === "at_rule") && (data.types[ss - 2] === "template" || data.types[ss - 2] === "liquid_start" || data.types[ss - 2] === "liquid_else" || data.types[ss - 2] === "liquid_end")) {
          ...
          data.token[ss - 1] = normalize2(`${data.token[ss - 1]}${data.token[ss]}`);
          data.types[ss - 1] = "selector";
          parse.splice({
            data,
            howmany: 1,
          ...
          do {
            ...
            if (data.begin[ss] === dd) {
              ...
              if (data.token[ss] === ":" && not(data.token[ss - 1], 59 /* SEM */)) {
                data.token[ss - 1] = normalize2(`${data.token[ss - 1]}:${data.token[ss + 1]}`);
                parse.splice({
                  data,
                  howmany: 2,
                  index: ss
      ...
      do {
        ...
        if (quote === NIL) {
          ...
          if (is(quote, 34 /* DQO */) || is(quote, 39 /* SQO */)) {
            quote = NIL;
          } else if (is(quote, 47 /* FWS */) && (is(b[a], 13 /* CAR */) || is(b[a], 10 /* NWL */))) {
            quote = NIL;
...
function markup2() {
  ...
  function onAttribute() {
    ...
    let plural = false;
    let attstart = false;
    let attcount = isIndex(parent + 1, "end");
