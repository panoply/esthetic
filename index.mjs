...
prettify.options.markup.preserveAttributeValues = false;
...
prettify.lexers.markup = function markup(source) {
  ...
  function tagName(tag2) {
    ...
    if (typeof tag2 !== "string")
      return "";
    space = tag2.replace(regex, "%").replace(/\s+/, " ").indexOf(" ");
    name = tag2.replace(regex, " ");
  ...
  function tag(end) {
    ...
    if (ltype === "comment" && (b[a] === "<" || b[a] === "{" && b[a + 1] === "%")) {
      ...
      do {
        ...
        if (ltype === "comment") {
          ...
          if (quote === "") {
            ...
            if (stest === true && /\s/.test(b[a]) === false && b[a] !== lastchar) {
              ...
              if (a < c) {
                do {
                  ...
                  if (options2.language !== "jsx" && (b[a] === "<" || b[a] === ">") && (quote === "" || quote === ">")) {
                    if (quote === "" && b[a] === "<") {
                      quote = ">";
                    } else if (quote === ">") {
                      if (b[a] === "<") ; else if (b[a] === ">") ;
                    ...
                    } else if (b[a] === "{" && b[a - 1] === "=" && options2.language !== "jsx") {
                      quote = "}";
                    } else if (b[a] === '"' || b[a] === "'") {
                      quote = b[a];
                      if (b[a - 1] === "=" && (b[a + 1] === "<" || b[a + 1] === "{" && b[a + 2] === "%" || /\s/.test(b[a + 1]) && b[a - 1] !== "=")) {
                        igcount = a;
                      }
                    } else if (b[a] === "(") {
                      quote = ")";
                  ...
                  } else if (options2.language === "jsx" && (quote === "}" || quote === "\n" && b[a] === "\n" || quote === "*/" && b[a - 1] === "*" && b[a] === "/")) {
                    if (quote === "}") {
                      if (b[a] === "{") {
                        bcount = bcount + 1;
                      } else if (b[a] === quote) {
            ...
            } else if (simple === true && options2.language === "jsx" && b[a] === "/" && (b[a + 1] === "*" || b[a + 1] === "/")) {
              stest = true;
              lex[lex.length - 1] = " ";
              attribute.push(b[a]);
              if (b[a + 1] === "*") {
            ...
            } else if ((b[a] === lastchar || end === "\n" && b[a + 1] === "<") && (lex.length > end.length + 1 || lex[0] === "]") && (options2.language !== "jsx" || jsxcount === 0)) {
              if (end === "\n") {
      ...
      if (tname === "xml") {
        html2 = "xml";
      } else if (html2 === "" && tname === "!DOCTYPE" && element.toLowerCase().indexOf("xhtml") > 0) {
        html2 = "xml";
      ...
      if (element.replace(start, "").replace(/^\s+/, "").indexOf("@ignorestart") === 0) {
        ...
        do {
          lex.push(b[a]);
          if (b[a] === "d" && lex.slice(lex.length - 10).join("") === "@ignoreend")
            break;
          a = a + 1;
        } while (a < c);
        do {
    ...
    cheat = (() => {
      ...
      if (html2 === "html") {
        if (element.charAt(0) === "<" && element.charAt(1) !== "!" && element.charAt(1) !== "?" && (parse.count < 0 || data.types[parse.count].indexOf("template") < 0)) {
        ...
        if (blocks.has(parse.structure[parse.structure.length - 1][0]) && peertest(tname.slice(1), parse.structure[parse.structure.length - 2][0])) {
          ...
          addHtmlEnd(0);
        } else if (tname.charAt(0) === "/" && blocks.has(parse.structure[parse.structure.length - 1][0]) && parse.structure[parse.structure.length - 1][0] !== tname.slice(1)) {
    ...
    if (/\bscript|style\b/.test(tname) && element.slice(element.length - 2) !== "/>" || /\bschema|style|stylesheet|javascript\b/.test(tname) && element.slice(element.length - 2) === "%}") {
      ...
      if (liquid2 === false && tname === "script" && (attValue === "" || attValue === "text/javascript" || attValue === "babel" || attValue === "module" || attValue === "application/javascript" || attValue === "application/x-javascript" || attValue === "text/ecmascript" || attValue === "application/ecmascript" || attValue === "text/jsx" || attValue === "application/jsx" || attValue === "text/cjs" || attValue === "application/json" || attValue === "application/ld+json")) {
        ext = true;
      } else if ((tname === "style" || tname === "stylsheet") && options2.language !== "jsx" && (attValue === "" || attValue === "text/css")) {
        ext = true;
...
prettify.beautify.markup = function markup2(options2) {
  ...
  type.idx = (index, name) => data.types[index].indexOf(name);
    ...
    function attribute() {
      ...
      do {
        ...
        if (data.types[a].indexOf("attribute") > 0) {
          ...
          if (options2.markup.preserveAttributes === true) {
            level.push(-10);
          } else if (options2.markup.forceAttribute === true || attStart === true || a < c - 1 && type.not(a + 1, "template_attribute") && type.idx(a + 1, "attribute") > 0) {
            level.push(lev);
          } else {
            level.push(lev);
          }
...
function options(rules) {
  ...
  for (const rule of keys(rules)) {
    if (((_b = (_a = definitions) == null ? void 0 : _a[rule]) == null ? void 0 : _b.lexer) === "all") {
...
export { exports_exports as default, definitions, language_exports as language };
