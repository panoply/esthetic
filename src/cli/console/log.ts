...
import { CLI } from '../run';
import { blue, blueBright } from 'ansis';
...
// @ts-expect-error
const TITLE = `${gray(`Æsthetic: ${VERSION}`)} - ${disclaimer}`;
const NWL = '\n';
 ...
 * Starting
 */
export function start (mode: string, options: CLI, count: number) {

  const files = count > 1 ? `${count} file` : `${count} files`;

  if (options.watch) {

    print(`${whiteBright(mode)}${colon}${cyan(files)}`);
 ...
 * File Changed
 */
export function language (oldLanguage: string, newLanguage: string) {

  print(`${neonCyan('change')}${colon}${blue(oldLanguage)}${arrow}${blueBright(newLanguage)}`);
