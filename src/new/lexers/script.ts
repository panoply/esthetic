...
import { Record, Structure } from '../prettify.d';
...
prettify.lexers.script = function script (source: string) {
  ...
  const options = prettify.rules.script;
  ...
  function asi (isEnd: boolean) {
    ...
    if (rules.script.variableList === 'list') vart.index[vart.len] = parse.count;
  ...
  function end (x: string) {
    ...
    if (vart.len > -1) {
      if (x === '}' && (
        (rules.script.variableList === 'list' && vart.count[vart.len] === 0) ||
    ...
    if (
      ...
      } else if (rules.script.endComma === 'never' && data.token[parse.count] === ',') {
  ...
  function general (starting: string, ending: string, type: string) {
    ...
    const qc = (rules.script.quoteConvert === undefined)
      ? 'none'
      : rules.script.quoteConvert;
