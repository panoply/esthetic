...
prettify.lexers.style = function style (source: string) {
  ...
  function value (val) {
    ...
    if (cc < leng) {
      do {
        ...
        } else if ((rules.style.noLeadZero ===
          false || rules.style
  ...
  const item = function lexer_style_item (type) {
    ...
    const selectorPretty = function lexer_style_item_selectorPretty (
      ...
      cc = parse.count;
      if (rules.style.sortSelectors === true && data
        .token[cc - 1] === ',') {
  ...
  do {
    if ((/\s/).test(b[a]) === true) {
      ...
      if (
        ...
        if (rules.style.sortProperties === true && b[
          a] === '}') {
  ...
  if (rules.style.sortProperties === true) parse.objectSort(data);
