...
export default (function style_init () {
  ...
  const style = function lexer_style (source) {
        ...
        do {
          if (options.lexerOptions.style.noLeadZero === true &&
            ...
            if (block !== '@' && block !== '{' && block !== '<') {
              if (options.lexerOptions.style.quoteConvert ===
                    'double') {
                values[cc] = values[cc]
                  .replace(/url\(/, 'url("')
                  .replace(/\)$/, '")');
    ...
    const buildtoken = function lexer_style_build () {
      ...
      let funk = null;
      const block = [];
      const qc = (options.lexerOptions.style.quoteConvert ===
              undefined)
        ? 'none'
        : options.lexerOptions.style.quoteConvert;
    ...
    const item = function lexer_style_item (type) {
      ...
      const selectorPretty = function lexer_style_item_selectorPretty (
        ...
        cc = parse.count;
        if (options.lexerOptions.style.objectSort === true && data
          .token[cc - 1] === ',') {
    ...
    const margin_padding = function lexer_style_marginPadding () {
      ...
      const removes = function lexer_style_marginPadding_removes () {
        ...
        const applyValues =
                function lexer_style_marginPadding_removes_applyValues (
                  ...
                  if (props.data[prop][0] === props.data[prop][1] && props
                    ...
                    prop][3] && props.data[prop][0] !== props.data[prop][1]) {
                    values =
        ...
        if (endtest === true) {
          if (begin < 0) {
            sparser.parseError =
                    'Brace mismatch. There appears to be more closing braces than starting braces.';
          } else {
    ...
    do {
      if ((/\s/).test(b[a]) === true) {
        ...
        if (
          ...
          if (options.lexerOptions.style.objectSort === true && b[
            a] === '}') {
