# Snapshot report for `tests/cases/liquid.test.mjs`

The actual snapshot is saved in `liquid.test.mjs.snap`.

Generated by [AVA](https://avajs.dev).

## Delimiter Trims Cases

> ### Snapshot 1
> Testing Liquid `delimiterTrims` in `force` mode. All Liquid output object tokens and tags will have trim dashes applied aside from those within strings.
> ```js
> {
>   "delimiterTrims": "force",
>   "delimiterSpacing": true
> }
> ```

    `{%- for x in no_trims -%}␊
    ␊
      <div class="{{ right_trims -}}">␊
    ␊
        <ul {%- if no_trims -%}{{- no_trims -}}{%- endif -%}>␊
    ␊
          <li class="{% if no_trims %}{{ right_trim -}}{% endif %}">␊
    ␊
            {{- full_trims | filter: 'something' -}}␊
            {{- left_trim | filter: 'something'-}}␊
            {{- right_trim | filter: 'something' -}}␊
            {{- no_trims | filter: 'something' -}}␊
    ␊
            {%- if no_trims -%}␊
              {{- no_trims | filter: 'something' -}}␊
            {%- endif -%}␊
    ␊
          </li>␊
    ␊
        </ul>␊
      </div>␊
    ␊
    {%- endif -%}`

> ### Snapshot 2
> Testing Liquid `delimiterTrims` in `force` mode. All Liquid output object tokens and tags will have trim dashes applied aside from those within strings.
> ```js
> {
>   "delimiterTrims": "force",
>   "delimiterSpacing": false
> }
> ```

    `{%- for x in no_trims -%}␊
    ␊
      <div class="{{ right_trims -}}">␊
    ␊
        <ul {%- if no_trims -%}{{- no_trims -}}{%- endif -%}>␊
    ␊
          <li class="{% if no_trims %}{{ right_trim -}}{% endif %}">␊
    ␊
            {{- full_trims | filter: 'something' -}}␊
            {{- left_trim | filter: 'something'-}}␊
            {{- right_trim | filter: 'something' -}}␊
            {{- no_trims | filter: 'something' -}}␊
    ␊
            {%- if no_trims -%}␊
              {{- no_trims | filter: 'something' -}}␊
            {%- endif -%}␊
    ␊
          </li>␊
    ␊
        </ul>␊
      </div>␊
    ␊
    {%- endif -%}`

> ### Snapshot 1
> Testing Liquid `delimiterTrims` in `strip` mode. All Liquid tags and output object tokens will have their trim dashes removed aside from those within strings, ie: attribute values.
> ```js
> {
>   "delimiterTrims": "strip",
>   "delimiterSpacing": true
> }
> ```

    `{% for x in full_trims %}␊
    ␊
      <div class="{{- full_trims -}}">␊
    ␊
        <ul {% if right_trim %}{{ full_trims }}{% endif %}>␊
    ␊
          <li class="{%- if full_trims -%}{{ right_trim -}}{% endif -%}">␊
    ␊
            {{ full_trims | filter: 'something' }}␊
            {{ left_trim | filter: 'something'}}␊
            {{ right_trim | filter: 'something' }}␊
            {{ full_trims | filter: 'something'}}␊
    ␊
            {% if full_trims %}␊
              {{ full_trims | filter: 'something' }}␊
            {% endif %}␊
    ␊
          </li>␊
    ␊
        </ul>␊
      </div>␊
    ␊
    {% endif %}`

> ### Snapshot 2
> Testing Liquid `delimiterTrims` in `strip` mode. All Liquid tags and output object tokens will have their trim dashes removed aside from those within strings, ie: attribute values.
> ```js
> {
>   "delimiterTrims": "strip",
>   "delimiterSpacing": false
> }
> ```

    `{% for x in full_trims  %}␊
    ␊
      <div class="{{- full_trims -}}">␊
    ␊
        <ul {% if right_trim %}{{ full_trims }}{% endif %}>␊
    ␊
          <li class="{%- if full_trims -%}{{ right_trim -}}{% endif -%}">␊
    ␊
            {{ full_trims | filter: 'something' }}␊
            {{ left_trim | filter: 'something'}}␊
            {{ right_trim | filter: 'something' }}␊
            {{ full_trims | filter: 'something'}}␊
    ␊
            {% if full_trims %}␊
              {{ full_trims | filter: 'something' }}␊
            {% endif %}␊
    ␊
          </li>␊
    ␊
        </ul>␊
      </div>␊
    ␊
    {% endif %}`

> ### Snapshot 1
> Testing Liquid `delimiterTrims` in `tags` mode. All Liquid tags will have trim dashes applied and output object tokens will be preserved.
> ```js
> {
>   "delimiterTrims": "tags",
>   "delimiterSpacing": true
> }
> ```

    `{%- for x in no_trims -%}␊
    ␊
      <div class="{{ right_trims -}}">␊
    ␊
        <ul {%- if no_trims -%}{{ no_trims }}{%- endif -%}>␊
    ␊
          <li class="{% if no_trims %}{{ right_trim -}}{% endif %}">␊
    ␊
            {{- full_trims | filter: 'something' -}}␊
            {{- left_trim | filter: 'something'}}␊
            {{ right_trim | filter: 'something' -}}␊
            {{ no_trims | filter: 'something' }}␊
    ␊
            {%- if full_trims -%}␊
              {{ no_trims | filter: 'something' }}␊
            {%- endif -%}␊
    ␊
          </li>␊
    ␊
        </ul>␊
      </div>␊
    ␊
    {%- endif -%}`

> ### Snapshot 2
> Testing Liquid `delimiterTrims` in `tags` mode. All Liquid tags will have trim dashes applied and output object tokens will be preserved.
> ```js
> {
>   "delimiterTrims": "tags",
>   "delimiterSpacing": false
> }
> ```

    `{%- for x in no_trims -%}␊
    ␊
      <div class="{{ right_trims -}}">␊
    ␊
        <ul {%- if no_trims -%}{{ no_trims }}{%- endif -%}>␊
    ␊
          <li class="{% if no_trims %}{{ right_trim -}}{% endif %}">␊
    ␊
            {{- full_trims | filter: 'something' -}}␊
            {{- left_trim | filter: 'something'}}␊
            {{ right_trim | filter: 'something' -}}␊
            {{ no_trims | filter: 'something' }}␊
    ␊
            {%- if full_trims -%}␊
              {{ no_trims | filter: 'something' }}␊
            {%- endif -%}␊
    ␊
          </li>␊
    ␊
        </ul>␊
      </div>␊
    ␊
    {%- endif -%}`

> ### Snapshot 1
> Testing Liquid `delimiterTrims` in `outputs` mode. All Liquid output object tags will have trim dashes applied but Liquid tag tokens will be preserved
> ```js
> {
>   "delimiterTrims": "outputs",
>   "delimiterSpacing": true
> }
> ```

    `{% for x in no_trims %}␊
    ␊
      <div class="{{ right_trims -}}">␊
    ␊
        <ul {%- if no_trims -%}{{- no_trims -}}{%- endif -%}>␊
    ␊
          <li class="{% if no_trims %}{{ right_trim -}}{% endif %}">␊
    ␊
            {{- full_trims | filter: 'something' -}}␊
            {{- left_trim | filter: 'something'-}}␊
            {{- right_trim | filter: 'something' -}}␊
            {{- no_trims | filter: 'something' -}}␊
    ␊
            {% if full_trims %}␊
              {{- no_trims | filter: 'something' -}}␊
            {% endif %}␊
    ␊
          </li>␊
    ␊
        </ul>␊
      </div>␊
    ␊
    {% endif %}`

> ### Snapshot 2
> Testing Liquid `delimiterTrims` in `outputs` mode. All Liquid output object tags will have trim dashes applied but Liquid tag tokens will be preserved
> ```js
> {
>   "delimiterTrims": "outputs",
>   "delimiterSpacing": false
> }
> ```

    `{% for x in no_trims %}␊
    ␊
      <div class="{{ right_trims -}}">␊
    ␊
        <ul {%- if no_trims -%}{{- no_trims -}}{%- endif -%}>␊
    ␊
          <li class="{% if no_trims %}{{ right_trim -}}{% endif %}">␊
    ␊
            {{- full_trims | filter: 'something' -}}␊
            {{- left_trim | filter: 'something'-}}␊
            {{- right_trim | filter: 'something' -}}␊
            {{- no_trims | filter: 'something' -}}␊
    ␊
            {% if full_trims %}␊
              {{- no_trims | filter: 'something' -}}␊
            {% endif %}␊
    ␊
          </li>␊
    ␊
        </ul>␊
      </div>␊
    ␊
    {% endif %}`

> ### Snapshot 1
> Testing Liquid `delimiterTrims` in `preserve` mode. All Liquid tags and output object tokens will have their trim dashes preserved.
> ```js
> {
>   "delimiterTrims": "preserve",
>   "delimiterSpacing": true
> }
> ```

    `{% for x in right_trim -%}␊
    ␊
      <div class="{{ no_trims }}">␊
    ␊
        <ul {% if right_trim -%}{{- full_trims -}}{% endif -%}>␊
    ␊
          <li class="{%- if full_trims -%}{{ right_trim -}}{% endif -%}">␊
    ␊
            {{ no_trims | filter: 'something' }}␊
            {{- left_trim | filter: 'something'}}␊
            {{ right_trim | filter: 'something' -}}␊
            {{- full_trims | filter: 'something'-}}␊
            {{- full_trims | filter: 'something'-}}␊
    ␊
            {%- if full_trima -%}␊
              {{ no_trims | filter: 'something' }}␊
            {%- endif -%}␊
    ␊
          </li>␊
    ␊
        </ul>␊
      </div>␊
    ␊
    {% endif -%}`

> ### Snapshot 2
> Testing Liquid `delimiterTrims` in `preserve` mode. All Liquid tags and output object tokens will have their trim dashes preserved.
> ```js
> {
>   "delimiterTrims": "preserve",
>   "delimiterSpacing": false
> }
> ```

    `{% for x in right_trim  -%}␊
    ␊
      <div class="{{ no_trims }}">␊
    ␊
        <ul {% if right_trim -%}{{- full_trims -}}{% endif -%}>␊
    ␊
          <li class="{%- if full_trims -%}{{ right_trim -}}{% endif -%}">␊
    ␊
            {{ no_trims | filter: 'something' }}␊
            {{- left_trim | filter: 'something'}}␊
            {{ right_trim | filter: 'something' -}}␊
            {{- full_trims | filter: 'something'-}}␊
            {{- full_trims | filter: 'something'-}}␊
    ␊
            {%- if full_trima -%}␊
              {{ no_trims | filter: 'something' }}␊
            {%- endif -%}␊
    ␊
          </li>␊
    ␊
        </ul>␊
      </div>␊
    ␊
    {% endif -%}`

## Preserve Inlined Outputs in Text Nodes

> ### Snapshot 1
> Testing inlined output object token tags. This test case ensures that text node structures and kept intact and output to the same line. The case is a basic sentence with output token surrounded by text content, the nodes should not be indented and break the imposed sentence.
> ```js
> {
>   "forceIndent": true
> }
> ```

    'Hello{{ object.prop.name | default: \'Sissel\' }} , Lorem ipsum dolor sit amet,{{ object.foo }} adipiscing elit, sed do eiusmod tempor {{ object.bar[0][1] }} ut labore et dolore magna {{ object.bar[0][2][\'string\']-}} . Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut {{- object.bar[0][3] -}} ex ea commodo consequat.'

> ### Snapshot 2
> Testing inlined output object token tags. This test case ensures that text node structures and kept intact and output to the same line. The case is a basic sentence with output token surrounded by text content, the nodes should not be indented and break the imposed sentence.
> ```js
> {
>   "forceIndent": true,
>   "preserveText": true
> }
> ```

    'Hello{{ object.prop.name | default: \'Sissel\' }} , Lorem ipsum dolor sit amet,{{ object.foo }} adipiscing elit, sed do eiusmod tempor {{ object.bar[0][1] }} ut labore et dolore magna {{ object.bar[0][2][\'string\']-}} . Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut {{- object.bar[0][3] -}} ex ea commodo consequat.'

> ### Snapshot 3
> Testing inlined output object token tags. This test case ensures that text node structures and kept intact and output to the same line. The case is a basic sentence with output token surrounded by text content, the nodes should not be indented and break the imposed sentence.
> ```js
> {
>   "forceIndent": true,
>   "preserveText": false
> }
> ```

    'Hello{{ object.prop.name | default: \'Sissel\' }} , Lorem ipsum dolor sit amet,{{ object.foo }} adipiscing elit, sed do eiusmod tempor {{ object.bar[0][1] }} ut labore et dolore magna {{ object.bar[0][2][\'string\']-}} . Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut {{- object.bar[0][3] -}} ex ea commodo consequat.'

> ### Snapshot 4
> Testing inlined output object token tags. This test case ensures that text node structures and kept intact and output to the same line. The case is a basic sentence with output token surrounded by text content, the nodes should not be indented and break the imposed sentence.
> ```js
> {
>   "forceIndent": false
> }
> ```

    `Hello {{ object.prop.name | default: 'Sissel' }}, Lorem ipsum dolor sit amet, {{ object.foo }}␊
    adipiscing elit, sed do eiusmod tempor {{ object.bar[0][1] }}␊
    ut labore et dolore magna {{ object.bar[0][2]['string']-}}␊
    . Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut {{- object.bar[0][3] -}}␊
    ex ea commodo consequat.`

> ### Snapshot 5
> Testing inlined output object token tags. This test case ensures that text node structures and kept intact and output to the same line. The case is a basic sentence with output token surrounded by text content, the nodes should not be indented and break the imposed sentence.
> ```js
> {
>   "forceIndent": false,
>   "preserveText": true
> }
> ```

    `Hello {{ object.prop.name | default: 'Sissel' }}, Lorem ipsum dolor sit amet, {{ object.foo }}␊
    adipiscing elit, sed do eiusmod tempor {{ object.bar[0][1] }}␊
    ut labore et dolore magna {{ object.bar[0][2]['string']-}}␊
    . Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut {{- object.bar[0][3] -}}␊
    ex ea commodo consequat.`

> ### Snapshot 6
> Testing inlined output object token tags. This test case ensures that text node structures and kept intact and output to the same line. The case is a basic sentence with output token surrounded by text content, the nodes should not be indented and break the imposed sentence.
> ```js
> {
>   "forceIndent": false,
>   "preserveText": false
> }
> ```

    `Hello {{ object.prop.name | default: 'Sissel' }}, Lorem ipsum dolor sit amet, {{ object.foo }}␊
    adipiscing elit, sed do eiusmod tempor {{ object.bar[0][1] }}␊
    ut labore et dolore magna {{ object.bar[0][2]['string']-}}␊
    . Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut {{- object.bar[0][3] -}}␊
    ex ea commodo consequat.`

> ### Snapshot 1
> Testing inlined output object token tags. This test case ensures that text node structures and kept intact and output to the same line. The case is little more detailed and attempts to be unpredicatable.
> ```js
> {
>   "forceIndent": true
> }
> ```

    `Hello{{ object.prop.name | default: 'Sissel' }} ,␊
      ␊
      Lorem ipsum dolor sit amet,{{ object.foo }} adipiscing elit,␊
      sed do eiusmod tempor {{ object.bar[0][1] }} ut labore et dolore magna {{ object.bar[0][2]['string']-}} .␊
      ␊
      Ut␊
      enim␊
      ad␊
      minim␊
      veniam,␊
      quis{{- object.bar[0][3] -}}␊
    {{- object.bar[0][3] -}}␊
    {{- object.bar[0][3] -}} nostrud␊
    exercitation␊
    ullamco{{- object.bar[0][3] -}}␊
    laboris nisi ut␊
    {{- object.bar[0][3] -}} ex ea commodo consequat.␊
    veniam,␊
    quis␊
    nostrud␊
    exercitation␊
    ␊
    extraneous spacing on same line {{- object.bar[0][3] -}} spaces will be stripped.␊
    ␊
    Lists␊
    ␊
    - {{- object.bar[0][3] -}} -␊
    {{ object.bar[0][3] -}} -␊
    {{- object.bar[0][3] }} -␊
    {{ object.bar[0][3] -}}`

> ### Snapshot 2
> Testing inlined output object token tags. This test case ensures that text node structures and kept intact and output to the same line. The case is little more detailed and attempts to be unpredicatable.
> ```js
> {
>   "forceIndent": true,
>   "preserveText": true
> }
> ```

    `Hello{{ object.prop.name | default: 'Sissel' }} ,␊
    ␊
    Lorem ipsum dolor sit amet,{{ object.foo }} adipiscing elit,␊
    sed do eiusmod tempor {{ object.bar[0][1] }} ut labore et dolore magna {{ object.bar[0][2]['string']-}} .␊
    ␊
    Ut␊
    enim␊
    ad␊
    minim␊
    veniam,␊
    quis{{- object.bar[0][3] -}}␊
    {{- object.bar[0][3] -}}␊
    {{- object.bar[0][3] -}} nostrud␊
    exercitation␊
    ullamco␊
    {{- object.bar[0][3] -}}␊
    laboris nisi ut␊
    {{- object.bar[0][3] -}} ex ea commodo consequat.␊
    veniam,␊
    quis␊
    nostrud␊
    exercitation␊
    ␊
    extraneous spacing on same line {{- object.bar[0][3] -}} spaces will be stripped.␊
    ␊
    Lists␊
    ␊
    - {{- object.bar[0][3] -}} -␊
    {{ object.bar[0][3] -}} -␊
    {{- object.bar[0][3] }} -␊
    {{ object.bar[0][3] -}}`

> ### Snapshot 3
> Testing inlined output object token tags. This test case ensures that text node structures and kept intact and output to the same line. The case is little more detailed and attempts to be unpredicatable.
> ```js
> {
>   "forceIndent": true,
>   "preserveText": false
> }
> ```

    `Hello{{ object.prop.name | default: 'Sissel' }} ,␊
      ␊
      Lorem ipsum dolor sit amet,{{ object.foo }} adipiscing elit,␊
      sed do eiusmod tempor {{ object.bar[0][1] }} ut labore et dolore magna {{ object.bar[0][2]['string']-}} .␊
      ␊
      Ut␊
      enim␊
      ad␊
      minim␊
      veniam,␊
      quis{{- object.bar[0][3] -}}␊
    {{- object.bar[0][3] -}}␊
    {{- object.bar[0][3] -}} nostrud␊
    exercitation␊
    ullamco{{- object.bar[0][3] -}}␊
    laboris nisi ut␊
    {{- object.bar[0][3] -}} ex ea commodo consequat.␊
    veniam,␊
    quis␊
    nostrud␊
    exercitation␊
    ␊
    extraneous spacing on same line {{- object.bar[0][3] -}} spaces will be stripped.␊
    ␊
    Lists␊
    ␊
    - {{- object.bar[0][3] -}} -␊
    {{ object.bar[0][3] -}} -␊
    {{- object.bar[0][3] }} -␊
    {{ object.bar[0][3] -}}`

> ### Snapshot 4
> Testing inlined output object token tags. This test case ensures that text node structures and kept intact and output to the same line. The case is little more detailed and attempts to be unpredicatable.
> ```js
> {
>   "forceIndent": false
> }
> ```

    `Hello {{ object.prop.name | default: 'Sissel' }},␊
      ␊
      Lorem ipsum dolor sit amet, {{ object.foo }}␊
    adipiscing elit,␊
    sed do eiusmod tempor {{ object.bar[0][1] }}␊
    ut labore et dolore magna {{ object.bar[0][2]['string']-}}.␊
    ␊
    Ut␊
    enim␊
    ad␊
    minim␊
    veniam,␊
    quis{{- object.bar[0][3] -}} {{- object.bar[0][3] -}} {{- object.bar[0][3] -}}␊
    nostrud␊
    exercitation␊
    ullamco{{- object.bar[0][3] -}}␊
    laboris nisi ut {{- object.bar[0][3] -}}␊
    ex ea commodo consequat.␊
    veniam,␊
    quis␊
    nostrud␊
    exercitation␊
    ␊
    extraneous spacing on same line {{- object.bar[0][3] -}}␊
    spaces will be stripped.␊
    ␊
    Lists␊
    ␊
    - {{- object.bar[0][3] -}}␊
    - {{ object.bar[0][3] -}}␊
    - {{- object.bar[0][3] }}␊
    - {{ object.bar[0][3] -}}`

> ### Snapshot 5
> Testing inlined output object token tags. This test case ensures that text node structures and kept intact and output to the same line. The case is little more detailed and attempts to be unpredicatable.
> ```js
> {
>   "forceIndent": false,
>   "preserveText": true
> }
> ```

    `Hello {{ object.prop.name | default: 'Sissel' }},␊
    ␊
    Lorem ipsum dolor sit amet, {{ object.foo }}␊
    adipiscing elit,␊
    sed do eiusmod tempor {{ object.bar[0][1] }}␊
    ut labore et dolore magna {{ object.bar[0][2]['string']-}}.␊
    ␊
    Ut␊
    enim␊
    ad␊
    minim␊
    veniam,␊
    quis␊
    {{- object.bar[0][3] -}} {{- object.bar[0][3] -}} {{- object.bar[0][3] -}}␊
    nostrud␊
    exercitation␊
    ullamco␊
    {{- object.bar[0][3] -}}␊
    laboris nisi ut {{- object.bar[0][3] -}}␊
    ex ea commodo consequat.␊
    veniam,␊
    quis␊
    nostrud␊
    exercitation␊
    ␊
    extraneous spacing on same line {{- object.bar[0][3] -}}␊
    spaces will be stripped.␊
    ␊
    Lists␊
    ␊
    - {{- object.bar[0][3] -}}␊
    - {{ object.bar[0][3] -}}␊
    - {{- object.bar[0][3] }}␊
    - {{ object.bar[0][3] -}}`

> ### Snapshot 6
> Testing inlined output object token tags. This test case ensures that text node structures and kept intact and output to the same line. The case is little more detailed and attempts to be unpredicatable.
> ```js
> {
>   "forceIndent": false,
>   "preserveText": false
> }
> ```

    `Hello {{ object.prop.name | default: 'Sissel' }},␊
      ␊
      Lorem ipsum dolor sit amet, {{ object.foo }}␊
    adipiscing elit,␊
    sed do eiusmod tempor {{ object.bar[0][1] }}␊
    ut labore et dolore magna {{ object.bar[0][2]['string']-}}.␊
    ␊
    Ut␊
    enim␊
    ad␊
    minim␊
    veniam,␊
    quis{{- object.bar[0][3] -}} {{- object.bar[0][3] -}} {{- object.bar[0][3] -}}␊
    nostrud␊
    exercitation␊
    ullamco{{- object.bar[0][3] -}}␊
    laboris nisi ut {{- object.bar[0][3] -}}␊
    ex ea commodo consequat.␊
    veniam,␊
    quis␊
    nostrud␊
    exercitation␊
    ␊
    extraneous spacing on same line {{- object.bar[0][3] -}}␊
    spaces will be stripped.␊
    ␊
    Lists␊
    ␊
    - {{- object.bar[0][3] -}}␊
    - {{ object.bar[0][3] -}}␊
    - {{- object.bar[0][3] }}␊
    - {{ object.bar[0][3] -}}`

## Multilined Tags

> ### Snapshot 1
> Test multiline tags. These are tags which have content indented onto a newline

    `{% render 'snippet' with products as product␊
    ␊
     , param_1: 'foo'␊
     , param_2: 'foo'␊
     , param_2: 'foo'␊
    ␊
     , param_2: 'foo'␊
     ,param_2: 'foo'␊
    ␊
     ,param_2: 'foo'␊
     param_2: 'foo', param_2: 'foo'␊
     , %}␊
    ␊
    ␊
    {% render 'snippet'␊
    ␊
     ␊
    ␊
     , param_1: 'foo'␊
     , param_2: 'foo'␊
     , param_2: 'foo'␊
    ␊
     , param_2: 'foo'␊
     ,param_2: 'foo'␊
    ␊
     ,param_2: 'foo'␊
     param_2: 'foo', param_2: 'foo'␊
     , %}`
